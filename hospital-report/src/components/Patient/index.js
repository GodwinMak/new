...
const Patient = () => {
  ...
  const handleNavigate = () => {
    navigate("/main/newpatient");
  };

  const handleAdmitPatient = async (user) => {
    console.log(user);
    await axios.get(`${url}/admiting/check/${user.patient_id}`).then((res) => {
      if (res.data.status) {
        dispatch({
          type: "GETPATIENT",
          payload: user,
        });
        navigate("/main/admitingpatient");
      }else{
        toast.error("Patient is aready Admited", toastOptions)
      }
    });
  };
    ...
    columnHelper.accessor("", {
      ...
      cell: (info) => (
        <button
          ...
          onClick={() => handleAdmitPatient(info.row.original)}
        >
          Admit Patient
        </button>
      ),
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(
          `${url}/patient?page=${meta.currentPage}&pageSize=${meta.pageSize}`
        );
    ...
    fetchData();
  }, [Data.full_name, dispatch, meta.currentPage, meta.pageSize]);
